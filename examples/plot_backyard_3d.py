...
class SimulationVisualizer:
    def __init__(self, frames_dir):
        ...
        self.slice_y = int(0.05 * self.ny)  # Horizontal slice near ground (5% of NY)
        self.slice_z = self.nz // 2  # Vertical slice at center
        
        # Backyard Scene Parameters
        # Load the backyard mask to visualize obstacles
        import jax.numpy as jnp
        self.backyard_mask = np.array(backyard_scene(self.nx, self.ny, self.nz))
    ...
    def setup_scene(self):
        ...
        for i in range(3):
            ...
            # Add Backyard Obstacles
            # Create a mesh from the backyard mask
            obstacle_grid = pv.ImageData(dimensions=(self.nx, self.ny, self.nz))
            obstacle_grid.point_data["Obstacle"] = self.backyard_mask.flatten(order='F').astype(float)
            
            # Extract obstacles (where mask is True) using contour
            obstacles = obstacle_grid.contour([0.5], scalars="Obstacle")
            if obstacles.n_points > 0:
                self.plotter.add_mesh(obstacles, color='saddlebrown', opacity=0.7, label='Backyard Obstacles')
            
            # Initial Slices
            self.update_subplot(i, scalars[i], cmaps[i])
            
            # Set camera AFTER meshes are added (critical for clipping range calculation)
            # Position camera to see entire domain from isometric-like view
            self.plotter.camera_position = [
                (self.nx * 3.5, self.ny * 4.5, self.nz * 3.5),  # Camera position (balanced zoom-out)
                (self.nx / 2, self.ny / 4, self.nz / 2),        # Focal point (looking at lower part of domain)
                (0, 1, 0)                                        # View up direction (Y is up)
            ]
            
            # Set wider field of view to prevent clipping at edges
            self.plotter.camera.view_angle = 50  # Default is ~30, increase to see more
    ...
    def add_widgets(self):
        ...
        self.plotter.add_slider_widget(
            y_slice_callback,
            [0, self.ny - 1],
            value=int(0.05 * self.ny),  # Start at 5% height (near ground)
            title="Y Slice (Horizontal)",
            pointa=(0.5, 0.9),
            pointb=(0.8, 0.9),
            style='modern'
        )
